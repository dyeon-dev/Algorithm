...
## 오답노트
 다시 풀어보았을 때 while문의 조건을 잘못써서 오류가 났다. 
 
 큐 조건문이라고 무작정 while(!q.isEmpty())를 쓰지말자.

 이 문제는 더해준 값을 다시 큐에 삽입하고, 결국엔 heap에 결과값인 하나만 남겨 출력해줘야 한다.
 따라서 while(q.size()!=1)으로 설정하고 heap 사이즈가 1이 될때까지 반목문을 수행한다.
